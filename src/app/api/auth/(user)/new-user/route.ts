...
async function POST(request: Request) {
  ...
  // })
    ...
    const response = Response.json(
      {error: 'User already exists', token: tokenUser, progreessCompletion: user.progressCompletion,
        redirectUrl: `/`
      },
      {status: 202, }
    ...
    const response = NextResponse.json(
      {sucess: 'true', redirectUrl: `auth/sign-in/step1_Profile?email=${body.email}`, token: token},
      {status: 201, }
    )
