...
export function ButtonSubmitOrOnClick({isPending, error,  Sucess, ...props}: ButtonProps) {
    return (
        <button {...props} className={button({...props})} > 
            {Sucess ? <Check className="w-5 h-8 mx-auto"/> : isPending ? <Loader /> : error ? <X className="w-5 h-8 mx-auto"/> : props.children}
        </button>
    )
}
