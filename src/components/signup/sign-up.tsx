...
export function SignUpPage() {
    ...
    const handleSubmit = async (formData: FormData) => {
        ...
        const {data,decodedToken, error, token: tokenSetCookie, progreessCompletion} = await NewUser(formData)
    ...
    return (
        <div className="w-[23rem] h-[33.125rem] text-black">
            ...
            {errorSenhaNotMatch && <p className="text-red-500 text-sm">As senhas não coincidem</p>}
           ...
           <div className="flex w-fit mx-auto text-Label gap-3 mt-3">
            ...
            <p className="text-[#4D4D4D] cursor-pointer underline underline-offset-8" onClick={setIsReadyAccount}>Log in</p>
           </div>
        </div>
    )
}